...
\begin{titlepage}
 \begin{center}
	...
	\begin{abstract}
    The main task of this experiment was to solder a PID-controller to regulate the temperature of a laser cavity in order to improve the stability of the laser.
  \end{abstract}
    \vspace{1cm}
...
The ability to control and modulate the output of a system or process in a precise way is of fundamental importance not only in modern day science, but also in engineering and high-tech industry. The PID-controller, which was explored in this experiment, is a reliable and useful example for a controller. In this case it was used to stabilized the frequency of a He-Ne laser, by controlling the temperature of the lasing cavity.

\section{Theoretical Background}
\label{theory}
\subsection{The Control Loop}
In this experiment we used a feedback loop, which is sketched in Fig. \ref{fig_control_loop}. The output of the system $X$ is the difference in intensity of two laser modes. It is compared to the desired value $W$, which is zero in this case. The resulting signal is fed into the controller, which in its output $Y$ drives a heating coil the He-Ne laser's cavity. The temperature is further influenced by outside influence, which is accounted by the error signal $Z$. The combined change in temperature causes the length of the cavity to change, thus changing the ratio in the intensities of the modes. This then leads back to a different output of the system, which closes the loop.
\begin{figure}[htp!]
    \centering
...
\begin{figure}[htp!]
  ...
  \begin{tikzpicture}
    \begin{axis}[
      no markers, domain=0:10, samples=100, axis lines*=left, xlabel=Frequency at \SI{633}{\nano \meter}, ylabel=Amplification,
...
\section{Conclusion}
%In this experiment we stabilized a laser using a PID controller, we checked
The main task of this experiment was to solder a working PID controller, which was done successfully. The stability measurements showed that the PID controller was working more stably than the mere P controller, which was to be expected. The PI controller has an even bigger stability than the PID according to our measurement, but as was explained above, this is likely due to a measurement error.
