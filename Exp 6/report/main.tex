...
\end{table}
The errors in the table were estimated with the error on the length $d$, which is the biggest source of uncertainty. Therefore we fitted the data first using the lower bound $d = 27$ cm, then again with the upper bound $d= 33$ cm, and we averaged the two results of the two fits, so we wrote the error as the relative distance divided by two. From the Mach number we can see that we are indeed in the supersonic regime. Moreover, the longitudinal temperature is between 6 and 32 K, as expected 
